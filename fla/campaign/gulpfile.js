    ...
    if (projectInfo.type == "canvas"){
        ...
        for (var i = 0; i < sceneList.length; i++) {
            ...
            gulp.src(jsList).pipe(babel({
                presets: ['@babel/env']
            })).pipe(uglify()).pipe(concat(sceneList[i] + projectInfo.suffixName)).pipe(gulp.dest("../../source/dist/src/" + sceneList[i]));
            //复制其他资源
            ...
            stream.pipe(vinylBuffer()).pipe(gulp.dest("../../source/dist/resource/" + sceneList[i]));
            stream.end();
            //fs.writeFile("./../../source/dist/resource/" + sceneList[i] + "/" + sceneList[i] + ".res.json", JSON.stringify(sceneInfo, null, ""),function (err) {});
            //压缩资源
            gulp.src(resList).pipe(gulpPngquant()).pipe(gulp.dest("../../source/dist/resource/" + sceneList[i]));
        }
        //复制其他资源
        if(isAll) {
            //复制libs库
            ...
            for (var i = 0; i < resList.length; i++) {
                gulp.src("resource/" + resList[i] + "/**/*").pipe(gulp.dest("../../source/dist/resource/" + resList[i]));
            }
            var otherFileList = getFiles("resource");
            for (var i = 0; i < otherFileList.length; i++) {
                gulp.src("resource/" + otherFileList[i]).pipe(gulp.dest("../../source/dist/resource"));
            }
        }
    }
});
...
function initMergeFile(){
    if(sceneIndex<sceneList.length) {
        ...
        resourceJSON = require("../../source/dist/resource/" + resourceItem + "/" + resourceItem + ".res.json");
        var jsURL="../../source/dist/src/" + resourceItem + "/" + resourceItem + projectInfo.suffixName;
...
function mergeFile() {
    ...
    rs.on("end",function(){
        if(resourceIndex==resourceJSON.length-1){
            //保存resource
            fs.writeFileSync("../../source/dist/resource/" + resourceItem + "/" + resourceItem + ".res.json",JSON.stringify(resourceJSON));
            var size=fs.statSync("../../source/dist/resource/" + resourceItem + "/" + resourceItem + ".res.json").size.toString();
            ...
            writeFile.end();
            //删除不需要的资源文件夹
            del("../../source/dist/resource/" + resourceItem,{
                force : true
            });
            //获取swf大小
            fileSizeArr[resourceItem] = fs.statSync("../../source/dist/src/" + resourceItem + "/" + resourceItem + projectInfo.suffixName).size;
            sceneIndex++;
