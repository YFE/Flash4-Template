...
gulp.task("prepare",["clean"],function () {
    if (projectInfo.type == "canvas") {
        ...
        for (var i = 0; i < sceneList.length; i++) {
            ...
            //合并压缩js
            gulp.src(jsList).pipe(browserify({insertGlobals: true, debug: false})).pipe(uglify()).pipe(concat(sceneList[i] + ".js")).pipe(gulp.dest("../../source/dist/src/" + sceneList[i]));
            //复制其他资源
            gulp.src(otherList).pipe(gulp.dest("../../source/dist/resource/" + sceneList[i]));
            //重写res.json文件
            sceneInfo.unshift({type: "javascript", src: "src/" + sceneList[i] + "/" + sceneList[i] + ".js"});
        ...
        //复制libs库
        // gulp.src("libs/*.js").pipe(gulp.dest("../../source/dist/libs"));
        //复制其他资源
...
gulp.task("packToOne",function () {
    if (projectInfo.type == "canvas"){
        ...
        for (var i = 0; i < sceneList.length; i++) {
            ...
            var resourceJson = require("../../source/dist/resource/" + item + "/" + item + ".res.json");
            var resObj = {};
            for (var j = resourceJson.length - 1; j >= 0; j--) {
                if (resourceJson[j].type != "javascript") {
                    ...
                    if (url.indexOf(".json") > 0) {
                        resObj[resourceJson[j].id] = require("../../source/dist/" + url);
                    } else {
                        var head;
                        ...
                        resObj[resourceJson[j].id] = head + base64.encodeFile("../../source/dist/" + url);
                    }
                }
            }
            var content = "annie.res." + item + "=" + JSON.stringify(resObj, null, "").replace(/"([\w\d_\$]+)"\:/g, "$1:") + ";" + fs.readFileSync("../../source/dist/src/" + sceneList[i] + "/" + sceneList[i] + ".js");
            //正则替换里面所有的
            fs.writeFile("../../source/dist/src/" + sceneList[i] + "/" + sceneList[i] + ".js", content, "utf8", function (err) {});
            //删除不需要的资源
